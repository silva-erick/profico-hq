...
class BaseCatarseCollectionApi:
    ...
    def execute(self, cod, desc, verbose = False, log_level = logging.WARNING):
        ...
        headers = self._get_headers(cod)
        params = self._get_params(cod)
        url = self._get_url()
        ...
        try:
            ...
            # for _ in range(1):
                ...
                prepped.headers[":Path"] = self._get_header_path(cod)
...
class CatarseFinishedProjects(BaseCatarseCollectionApi):
    ...
    def _get_header_path(self, cod):
        return f'/finished?state_order.asc%2Cstate.desc%2Cpledged.desc&mode=not.eq.sub&category_id=eq.{cod}'
    
    def execute(self, cod, desc, verbose=False, log_level=logging.WARNING):
        res = super().execute(cod, desc, verbose, log_level)
        if not res.sucesso:
            print("?", end='')
        else:
            ...
            for fin in res.resultado:
                ...
                if not os.path.exists(f"../../dados/brutos/catarse/{desc}/{project_id}.json"):
                    ...
                    if project_success:
                        data_file = f"../../dados/brutos/catarse/{desc}/{project_id}.json"
                        # Using a JSON string
...
def collect_catarse(verbose):
    if not os.path.exists("../../dados/brutos/catarse/esporte/"):
        os.makedirs("../../dados/brutos/catarse/esporte/")
    if not os.path.exists("../../dados/brutos/catarse/jornalismo"):
        os.makedirs("../../dados/brutos/catarse/jornalismo")
    ...
    for cod, desc in cats.items():
        for k, api in apiset.items():
            ...
            if verbose:
                ...
                data_file = f"../../dados/brutos/catarse/{k}_{desc}.json"
                with open(data_file, 'w') as json_file:
